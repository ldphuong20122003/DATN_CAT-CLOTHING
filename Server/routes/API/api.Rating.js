...
router.get('/', async(req, res, next) =>{
  try {
    const snapshot = await db.collection('Assess').get();
    const data = [];
  ...
  router.post('/add',async(req,res,next)=>{
    try {
      ...
      const docRef = await db.collection('Assess').add(data);
  ...
  router.delete('/delete/:id',async(req,res,next)=>{
    try {
      ...
      await db.collection('Assess').doc(docId).delete();
  ...
  router.put('/update/:id',async(req,res,next)=>{
    try {
      ...
      await db.collection('Assess').doc(docId).set(newData, { merge: true });
